...
mod tests {
    ...
    fn test_css_transition_generation() {
        ...
        let css = config.to_css_transition("opacity");
    ...
    fn test_presets() {
        ...
        assert_eq!(fade_in.transition_type, TransitionType::Fade);
        assert_eq!(fade_in.duration, Duration::Mid);

        let slide_in = presets::slide_in_up();
        assert_eq!(slide_in.direction, Some(Direction::Up));

        let pulse = presets::pulse();
        assert!(pulse.infinite);
    }
}
