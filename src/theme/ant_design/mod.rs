...
impl DesignTokens for AntDesignTheme {
...
impl ThemeConfig for AntDesignTheme {
    fn name(&self) -> &str {
        &self.name
    }
...
impl AntDesignThemePresets {
    /// 获取所有预设主题
    pub fn all() -> Vec<AntDesignTheme> {
        vec![
            AntDesignTheme::light(),
            AntDesignTheme::dark(),
            AntDesignTheme::compact(),
        ]
    }
    ...
    /// 根据名称获取主题
    pub fn get_by_name(name: &str) -> Option<AntDesignTheme> {
        match name {
...
impl Default for AntDesignTheme {
    fn default() -> Self {
        Self::light()
    }
}
...
mod tests {
    ...
    fn test_theme_presets() {
        let all_themes = AntDesignThemePresets::all();
        assert_eq!(all_themes.len(), 3);

        let default_theme = AntDesignThemePresets::default();
        assert!(!default_theme.is_dark_theme());

        let light_theme = AntDesignThemePresets::get_by_name("light").unwrap();
        assert!(!light_theme.is_dark_theme());

        assert!(AntDesignThemePresets::get_by_name("nonexistent").is_none());
    }

    #[test]
    fn test_design_tokens() {
        let theme = AntDesignTheme::light();
    ...
    fn test_custom_theme() {
        let custom = AntDesignThemePresets::create_custom_theme(
