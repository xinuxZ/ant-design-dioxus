...
use crate::theme::core::{ColorConfig, ColorPalette, ColorType, RgbColor};
...
/// 实现 ColorConfig trait
impl ColorConfig for AntDesignColors {
    /// 创建浅色主题颜色配置
    fn light() -> Self {
    ...
    fn get_color(&self, color_type: ColorType) -> Option<RgbColor> {
        AntDesignColors::get_color(self, color_type)
    }

    /// 获取指定类型的调色板
    fn get_palette(&self, color_type: ColorType) -> Option<&ColorPalette> {
        AntDesignColors::get_palette(self, color_type)
    }

    /// 生成 CSS 变量
    fn to_css_variables(&self) -> HashMap<String, String> {
        AntDesignColors::to_css_variables(self)
    }

    /// 生成 CSS 样式
    fn generate_css(&self) -> String {
        AntDesignColors::generate_css(self)
    }
