...
pub fn TreeSelect(props: TreeSelectProps) -> Element {
    ...
    // 处理节点选择
    let handle_node_select = std::rc::Rc::new({
        let mut selected_values = selected_values.clone();
        let mut open = open.clone();
        move |node_value: String| {
            let mut new_values = selected_values();

            if props.multiple {
                if new_values.contains(&node_value) {
                    new_values.retain(|v| v != &node_value);
                } else {
                    new_values.push(node_value.clone());
                }
            } else {
                new_values = vec![node_value.clone()];
                open.set(false);
            }
            ...
            if let Some(on_change) = &props.on_change {
                ...
                on_change(new_values, vec![node_value.clone()], labels);
            }

            if let Some(on_select) = &props.on_select {
                let extra = HashMap::new(); // 简化实现
                on_select(node_value, true, extra);
            }
        }
    });
                            ...
                            if !props.disabled {
                                span {
                                    ...
                                    onclick: move |_| {
                                        ...
                                        selected_values.set(new_values.clone());
                                        if let Some(on_change) = &props.on_change {
                                            on_change(new_values, vec![], HashMap::new());
                                        }
                                    },
                                    "×"
                                }
                ...
                div {
                    ...
                    div {
                        ...
                        onclick: {
                            ...
                            let handle_node_select = (&handle_node_select).clone();
                            move |_| {
                                if !node.disabled {
                                    handle_node_select(node_value.clone());
                                }
                            }
                        },
                                ...
                                div {
                                    ...
                                    div {
                                        ...
                                        onclick: {
                                            ...
                                            move |_| {
                                                if !child.disabled {
                                                    handle_node_select(child_value.clone());
                                                }
                                            }
                                        },
