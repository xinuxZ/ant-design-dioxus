...
pub fn Tag(props: TagProps) -> Element {
    ...
    // 注册基础样式
    {
        ...
        use_effect(move || {
            ...
            // 注册变体样式
            match variant {
                TagVariant::Outlined => { let _ = css!(styles.variants.outlined); },
                TagVariant::Borderless => { let _ = css!(styles.variants.borderless); },
                _ => { let _ = css!(styles.variants.filled); }
            }
            
            // 注册颜色样式
            match color {
                TagColor::Primary => { let _ = css!(styles.colors.primary); },
                TagColor::Success => { let _ = css!(styles.colors.success); },
                TagColor::Warning => { let _ = css!(styles.colors.warning); },
                TagColor::Error => { let _ = css!(styles.colors.error); },
                TagColor::Info => { let _ = css!(styles.colors.info); },
                TagColor::Processing => { let _ = css!(styles.colors.processing); },
                TagColor::Preset(preset) => {
                    match preset {
                        TagPresetColor::Magenta => { let _ = css!(styles.colors.preset_colors.magenta); },
                        TagPresetColor::Red => { let _ = css!(styles.colors.preset_colors.red); },
                        TagPresetColor::Volcano => { let _ = css!(styles.colors.preset_colors.volcano); },
                        TagPresetColor::Orange => { let _ = css!(styles.colors.preset_colors.orange); },
                        TagPresetColor::Gold => { let _ = css!(styles.colors.preset_colors.gold); },
                        TagPresetColor::Lime => { let _ = css!(styles.colors.preset_colors.lime); },
                        TagPresetColor::Green => { let _ = css!(styles.colors.preset_colors.green); },
                        TagPresetColor::Cyan => { let _ = css!(styles.colors.preset_colors.cyan); },
                        TagPresetColor::Blue => { let _ = css!(styles.colors.preset_colors.blue); },
                        TagPresetColor::GeekBlue => { let _ = css!(styles.colors.preset_colors.geekblue); },
                        TagPresetColor::Purple => { let _ = css!(styles.colors.preset_colors.purple); },
                    }
                },
                _ => { let _ = css!(styles.colors.default); }
            }
            
            // 注册尺寸样式
            match size {
                TagSize::Small => { let _ = css!(styles.sizes.small); },
                TagSize::Large => { let _ = css!(styles.sizes.large); },
                _ => { let _ = css!(styles.sizes.middle); }
            }
        });
    }
