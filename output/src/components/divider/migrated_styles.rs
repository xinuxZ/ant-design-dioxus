...
use super::types::*;
use css_in_rust::css;
use super::styles::DividerStyleGenerator;
...
/// Divider 样式生成器（迁移版本）
#[derive(Debug, Clone)]
pub struct MigratedDividerStyleGenerator {
...
impl MigratedDividerStyleGenerator {
    pub fn new() -> Self {
        Self {
                ...
                if self.has_text {
                    ...
                    match self.orientation {
                        DividerOrientation::Left => {
                            classes.push(format!("{}-divider-with-text-left", self.prefix_cls))
                        }
                        DividerOrientation::Center => {
                            classes.push(format!("{}-divider-with-text-center", self.prefix_cls))
                        }
                        DividerOrientation::Right => {
                            classes.push(format!("{}-divider-with-text-right", self.prefix_cls))
                        }
                    }
                }
            }
        ...
        if let Some(margin) = &self.orientation_margin {
            if self.has_text && matches!(self.divider_type, DividerType::Horizontal) {
                ...
                let selector = match self.orientation {
                    DividerOrientation::Left => format!(
                        ".{}-divider-with-text-left .{}-divider-inner-text",
                        self.prefix_cls, self.prefix_cls
                    ),
                    DividerOrientation::Right => format!(
                        ".{}-divider-with-text-right .{}-divider-inner-text",
                        self.prefix_cls, self.prefix_cls
                    ),
                    DividerOrientation::Center => format!(
                        ".{}-divider-with-text-center .{}-divider-inner-text",
                        self.prefix_cls, self.prefix_cls
                    ),
                };
                ...
                let style = match self.orientation {
                    DividerOrientation::Left => {
                        format!("{} {{ margin-left: {}; }}", selector, margin_value)
                    }
                    DividerOrientation::Right => {
                        format!("{} {{ margin-right: {}; }}", selector, margin_value)
                    }
                    DividerOrientation::Center => format!(
                        "{} {{ margin-left: {}; margin-right: {}; }}",
                        selector, margin_value, margin_value
                    ),
                };
